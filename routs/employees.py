...
async def get_all_employees()-> List[Employee]:
    return globals.employees.json_array

@router.get("/employee/{employee_id}" ,tags = ["employee"])
async def get_employee_by_id(employee_id) -> List[Employee]:
    emp = list(filter(lambda x :str(x["employee_id"])==employee_id,globals.employees.json_array))
    return emp
...
async def add_new_employee(emp :Employee ):
    if len(await get_employee_by_id(emp.employee_id)) == 0:
        res = await globals.employees.append_list(emp)
        return res
    else:
...
async def update_employee(employee_id:int ,emp: Employee):
    ...
    res = await globals.employees.update_list({"employee_id":employee_id},dict(emp))
    return res
...
async def read_data():
    response = FileResponse(globals.employees.csv_file_path, media_type="text/csv")
...
def get_employee_html():
    return json2html.convert(json=globals.employees.json_array, table_attributes="id=\"info-table\" border=\"1\"")